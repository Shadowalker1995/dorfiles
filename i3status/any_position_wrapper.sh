...
function hey_man {
  ...
  if [ $rand_val == 1 ] ; then
    local json='{ "full_text": "Debug The World!", "color": "#00FF00" }'
    json_array=$(update_holder holder__hey_man "$json")
  elif [ $rand_val == 0 ] ; then
    local json='{ "full_text": "Debug The World!", "color": "#FF0000" }'
    json_array=$(update_holder holder__hey_man "$json")
  else
...
i3status -c ~/.config/i3status/i3status.conf | (read line; echo "$line"; read line ; echo "$line" ; read line ; echo "$line" ; while true
do
  read line
